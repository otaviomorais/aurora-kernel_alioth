...
/* ---- auto-install embedded ksud (legacy 4.19 flow) ---- */
...
static bool ksu_ksud_already_installed(void)
{
	struct path p;
	struct inode *inode;
	bool ok = false;

	if (kern_path(KSUD_PATH, LOOKUP_FOLLOW, &p))
		return false;
	inode = d_inode(p.dentry);
	if (inode && i_size_read(inode) == (loff_t)ksud_size)
		ok = true;
	path_put(&p);
	return ok;
}

static int ksu_extract_ksud(void *unused)
{
	struct file *f;
	mm_segment_t old_fs;
	loff_t pos = 0;
	...
	for (tries = 0; tries < 240; tries++) {
		struct path p;
		if (!kern_path("/data/adb", LOOKUP_FOLLOW, &p)) {
			path_put(&p);
			break;
		}
		msleep(500);
	...
	if (tries >= 240) {
		pr_err("ksu_install: /data/adb never appeared\n");
		return 0;
	}
	if (ksu_ksud_already_installed()) {
		pr_info("ksu_install: ksud v-matching already present\n");
		return 0;
	}

	old_fs = get_fs();
	set_fs(KERNEL_DS);
	f = filp_open(KSUD_PATH, O_WRONLY | O_CREAT | O_TRUNC, 0755);
	if (IS_ERR(f)) {
		set_fs(old_fs);
		pr_err("ksu_install: open failed %ld\n", PTR_ERR(f));
		return 0;
	}
	while (off < ksud_size) {
		size_t chunk = min_t(size_t, ksud_size - off, 65536);
		ssize_t written = kernel_write(f, ksud + off, chunk, &pos);
		if (written <= 0) {
			pr_err("ksu_install: write failed at %zd (%zd)\n", off, written);
			break;
		}
		off += written;
	}
	...
	{
		struct path kp;
		if (!kern_path(KSUD_PATH, LOOKUP_FOLLOW, &kp)) {
			struct iattr ia = { .ia_valid = ATTR_MODE, .ia_mode = 0755 };
			notify_change2(kp.mnt, kp.dentry, &ia, NULL);
			path_put(&kp);
		}
	}
	set_fs(old_fs);
	pr_info("ksu_install: ksud extracted %zd/%u bytes\n", off, ksud_size);
	return 0;
}
...
void ksu_ksud_init()
{
	kthread_run(ksu_extract_ksud, NULL, "ksu_install");
#ifdef CONFIG_KSU_KPROBES_HOOK
	int ret;
